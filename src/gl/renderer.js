...
/**
 * `mipmap` matters for any tile that is REPEATED across the screen. The grunge
 * and grain tiles are 512px squares scaled to cover the viewport several times
 * over, which puts them well under one texel per device pixel - and the fine
 * octave the letter shader samples is finer still, around 2.8 texels per pixel.
 * Minified that hard with a plain LINEAR filter and no mip chain, the GPU
 * point-samples one texel out of every three and the tile aliases into
 * PER-PIXEL noise: the letters come out covered in hard flecks that no amount
 * of dialling the wear amplitude down will remove, because the problem is
 * sampling, not amplitude. With a mip chain the same fetch averages the texels
 * it is skipping and the tile reads as the abrasion it actually is.
 */
export function texture(gl, { wrap = 'clamp', filter = 'linear', mipmap = false } = {}) {
  const t = gl.createTexture();
  const W = wrap === 'repeat' ? gl.REPEAT : gl.CLAMP_TO_EDGE;
  const F = filter === 'nearest' ? gl.NEAREST : gl.LINEAR;
  const MIN = mipmap && filter !== 'nearest' ? gl.LINEAR_MIPMAP_LINEAR : F;
...
export function upload(gl, tex, source, flipY = false) {
  // textures created with { mipmap: true } need their chain rebuilt on every
  // upload, or the sampler falls back to an incomplete texture and draws black
  ...
  if (tex.__mip) gl.generateMipmap(gl.TEXTURE_2D);
