...
export class Stage {
  constructor(canvas) {
    ...
    this.tex = {
      glyph: texture(gl),
      grunge: texture(gl, { wrap: 'repeat', mipmap: true }),
      grain: texture(gl, { wrap: 'repeat', mipmap: true }),
      hero: texture(gl),
    };
    ...
    // and it had to be. The synthesised grunge tile is almost entirely HIGH
    // frequency (its fine-detail std is 35.7 of a total 46.5), because
    // build_grunge high-passes it twice. Run at the old 0.44/2.1 the letters
    // came out covered in hard flecks: 5.2% of pixels clamped to pure black and
    // 3.1% to pure white, giving a fine-detail amplitude of 0.167 against the
    // 0.038 measured off the reference artwork's own ink - about 4.4x too much.
    // The real culprit turned out to be sampling, not amplitude: the tile had
    // no mip chain, so it aliased into per-pixel noise (see renderer.js
    // texture()). With mipmaps in place these values land at per-pixel detail
    // 4.0 against the reference artwork's 4.3 - worn paint, no speckle.
    this.wear = 0.26;
    this.wearGain = 1.15;
    this.wearScale = 3.2;
  }
