...
export async function initGallery() {
  ...
  const cards = CARDS.map((c, i) => {
    // An anchor, not a button: these go somewhere, so the browser should treat
    // them as links - middle-click, cmd-click, "copy link address" and the
    // status-bar preview all come free, and a screen reader announces them as
    // links rather than controls.
    ...
    el.setAttribute('aria-label', `${c.title} — opens on ${c.host}`);
    ...
    el.innerHTML = `<span class="g-card__in">`
      ...
      + `<span class="g-card__go" aria-hidden="true">${c.host}`
      + `<svg viewBox="0 0 16 16"><path d="M4 12 L12 4 M6 4 H12 V10" fill="none"`
      + ` stroke="currentColor" stroke-width="1.5"/></svg></span></span>`;
    deck.appendChild(el);
    return el;
  });
