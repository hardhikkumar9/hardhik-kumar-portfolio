...
async function main() {
  ...
  // loopFade 0: the hero clip is encoded as a ping-pong (forward then
  // reversed) by tools/build_media.py, so its last frame IS its first frame
  // and there is no cut to hide. The old 0.7s dissolve faded him to fully
  // transparent for over a second on every wrap, which read as the figure
  // blurring out roughly every seven seconds.
  app.clips.hero = mk('hero', 0);
  ...
  initUniverse().then((u) => { app.universe = u; })
    .catch((e) => console.warn('[hardhik] universe unavailable:', e.message));
  initChrono().then((c) => { app.chrono = c; })
    .catch((e) => console.warn('[hardhik] chrono unavailable:', e.message));
  initGallery().then((g) => { app.gallery = g; })
    .catch((e) => console.warn('[hardhik] gallery unavailable:', e.message));
  initFinale().then((f) => { app.finale = f; })
    .catch((e) => console.warn('[hardhik] finale unavailable:', e.message));
...
function degrade(reason) {
  console.warn('[hardhik] falling back:', reason);
  root.classList.remove('is-booting');
  root.classList.add('is-fallback');
  boot.classList.add('is-done');
  for (const [, name] of CUES) root.classList.add(`is-${name}`);
  document.querySelector('.stage-wrap').insertAdjacentHTML('afterbegin',
    '<div class="fallback"><p>HARDHIK</p>'
