...
the repository or profile it describes.
...
| | |
|---|---|
| `index.html`, `src/` | the site — markup, scene modules, GLSL, stylesheets |
| `public/` | everything it serves: media, fonts, sprites, textures |
| `assets_src/` | the generated masters each asset is built from |
| `tools/` | the asset pipeline (see `tools/` docstrings, they carry the reasoning) |
| `docs/` | the generation prompts and the outstanding-questions list |

### A note on what is not committed

This engine shipped with a previous client's reference photography, video and
typography. None of it is ours to redistribute, so `.gitignore` keeps all of it
out. The practical consequence: `build_grunge()` in `tools/build_media.py` and
the `extract_s*.py` scripts read those references and cannot be re-run from a
fresh clone. Everything the site actually serves is committed, and the pipeline
for this site's own material — `prep_hero`, `prep_years`, `prep_panels`,
`prep_finale`, `build_media`, `track` — runs fine without them.

**Scene 1 — the opening.** Black screen → the man walks out of the dark → the
HARDHIK wordmark materialises behind him → chips, arrows and dots snap into
place → header draws itself in → the composition settles and breathes.

**Scene 2 — the analyst's toolkit.** The supplied film, played as the section:
...
**As shipped, scene four IS its supplied film** — the same treatment scene two
gets. `src/scene4/boot4.js` is the whole runtime: entrance from black,
loop-seam veil, visibility-gated decoding. The amphitheatre, the dashboard
screens, the figure and the lit floor circle are all in the footage; the DOM
adds only the chapter title and the project list a screen reader and a search
engine need.

Everything below describes the RECONSTRUCTED WebGL amphitheatre built from the
still reference (gallery.js, layout4.js, timeline4.js, and the twelve DOM card
buttons). It is parked, not deleted — restore boot4.js's old implementation and
the markup to bring back the hoverable deck. Note that its card geometry is
measured against the ORIGINAL reference art, so a different poster needs every
box in `layout4.js` re-measured before any of it lines up.
...
**The wordmark here is pixels, not live type.** The hero renders its letters
every frame; the footer's are part of the plate, set by `tools/prep_finale.py`
in Anton at the ratio measured off the reference's own stems - 1016 x 739 in a
1600x900 frame, so **1.375**, not the hero's 3.121. They are filled with the
gradient sampled from the reference and multiplied by the site's distress tile.
...
Anton, horizontally solved to the reference's 3.121 width:cap ratio; `HARDHIK`
is seven letters like the wordmark the art was measured from, so the geometry
carried over untouched) into a
...
tools/extract_fin.py       cuts a COMBINED footer poster into plate + man
                           (hardcoded to the original artwork - superseded)
tools/prep_finale.py       assembles the finale from separate layers, and
                           sets the footer wordmark in real Anton
src/scene6/timeline6.js    every beat of the closing shot
src/scene4/boot4.js        scene-4 lifecycle: visibility, loop seam
tools/prep_hero.py         Flow export -> matte-ready plate (delogo, shadows)
tools/prep_years.py        the six timeline photos -> public/years/
