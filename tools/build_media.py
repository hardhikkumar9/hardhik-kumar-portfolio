...
# The walk is trimmed at 163: from that frame the leading shoe is clipped by
# the bottom edge, and the site anchors him by his FEET, so a clipped foot
# makes the tracked box bottom out and his scale visibly pops.
CLIPS = [
    ("hero", "v1.mp4", "v1", 720, 1280, 0, 163),
    # ("letter_g", "v2.mp4", "v2", 540, 960, 0, 240),
    # ("letter_h", "v3.mov", "v3", 540, 960, 0, 190),
]


def stabilise(path, w):
    """Hold the subject at one size, in one place, for the whole clip.

    He walks TOWARD camera, so he grows through the shot - on the page that
    reads as the hero slowly zooming, which fights the settled composition the
    scene is built around. The walk itself is worth keeping; the travel is not.

    Each frame's subject box comes from the packed alpha, is smoothed over time
    so matte noise does not add jitter, and the frame is then scaled and
    translated so the box lands on the MEDIAN box of the whole clip. Median,
    not max or min: it splits the correction so the worst upscale is about 1.3x
    rather than 1.6x, and nothing is softened more than it has to be.

    Both halves get the same affine, or colour and matte would part company.
    """
    ...
    box = []
    for fr in frames:
        ys, xs = np.nonzero(fr[:, w:, 0] > 110)
        if len(ys) < 400:
            box.append(box[-1] if box else (w * 0.5, h * 0.95, h * 0.6))
            continue
        box.append(((xs.min() + xs.max()) * 0.5, float(ys.max()),
                    float(ys.max() - ys.min())))
    b = np.array(box, np.float32)

    # a wide moving average: we want the slow drift of the walk removed, not
    # the per-frame bob, which is him actually walking
    k = 15
    pad = np.pad(b, ((k, k), (0, 0)), mode="edge")
    sm = np.stack([np.convolve(pad[:, i], np.ones(k * 2 + 1) / (k * 2 + 1),
                               "same")[k:-k] for i in range(3)], 1)
    tgt = np.median(sm, axis=0)

    proc = subprocess.Popen(
        ["ffmpeg", "-v", "error", "-y", "-f", "rawvideo", "-pix_fmt", "bgr24",
         "-s", "%dx%d" % (frames[0].shape[1], h), "-r", "24", "-i", "-", "-an",
         "-c:v", "libx264", "-preset", "slow", "-crf", "18", "-pix_fmt",
         "yuv420p", "-color_range", "pc", path + ".st.mp4"],
        stdin=subprocess.PIPE)
    for fr, (cx, feet, bh) in zip(frames, sm):
        sc = float(tgt[2] / max(bh, 1.0))
        out = np.zeros_like(fr)
        for x0 in (0, w):                       # colour half, then matte half
            M = np.float32([[sc, 0, tgt[0] - cx * sc],
                            [0, sc, tgt[1] - feet * sc]])
            out[:, x0:x0 + w] = cv2.warpAffine(
                fr[:, x0:x0 + w], M, (w, h), flags=cv2.INTER_CUBIC,
                borderMode=cv2.BORDER_CONSTANT, borderValue=(0, 0, 0))
        proc.stdin.write(out.tobytes())
    proc.stdin.close()
    proc.wait()
    os.replace(path + ".st.mp4", path)
    print("  stabilised: subject held at h=%.0fpx, feet y=%.0f (was %.0f..%.0f)"
          % (tgt[2], tgt[1], b[:, 2].min(), b[:, 2].max()))


def pingpong(path):
    """Re-encode a clip as forward-then-reversed, so it loops with no cut.

    The walk starts far and ends close, so its wrap is a hard jump in scale.
    The runtime used to hide that by dissolving the figure to nothing across
    the seam - which, on a 6.8s clip, meant he vanished and re-materialised
    roughly every seven seconds. A ping-pong has no seam to hide: the last
    frame IS the first frame, so the loop is continuous and loopFade goes to 0.

    Costs double the frames, and the track is rebuilt over the new length by
    tools/track.py afterwards, so the feet anchor follows the reversed half.
    """
    tmp = path + ".pp.mp4" if path.endswith(".mp4") else path + ".pp.webm"
    enc = (["-c:v", "libx264", "-preset", "slow", "-crf", "18",
            "-pix_fmt", "yuv420p", "-color_range", "pc", "-movflags", "+faststart"]
           if path.endswith(".mp4") else
           ["-c:v", "libvpx-vp9", "-crf", "26", "-b:v", "0", "-row-mt", "1",
            "-cpu-used", "3", "-pix_fmt", "yuv420p"])
    subprocess.run(["ffmpeg", "-v", "error", "-y", "-i", path, "-an",
                    "-filter_complex",
                    # trim one frame off each end of the reversed half so the
                    # turnaround frames are not held for two frames each
                    "[0:v]split[a][b];[b]reverse,trim=start_frame=1[r];"
                    "[a][r]concat=n=2:v=1[v]",
                    "-map", "[v]"] + enc + [tmp], check=True)
    os.replace(tmp, path)
...
# Every Flow export carries a sparkle watermark burned into the same fixed spot
# of every frame. Located by averaging all frames and thresholding: on a busy
# scene the mark is the only compact bright feature that survives the average.
# Both 1920x1080 films put it in the same place. Box carries a little margin.
FILM_DELOGO = "delogo=x=1698:y=858:w=84:h=84"

# scenes that ARE their supplied film: (source in assets_src, output stem)
FILMS = [
    ("universe_src.mp4", "universe"),
    ("gallery_src.mp4", "gallery"),
]


def stage_film(srcfile, name):
    """A scene that plays its supplied film directly.

    Only the watermark is painted out; everything else about the picture is
    left alone. That forces a re-encode rather than a stream copy, so the CRF
    is kept low - these are the assets whose grade no shader touches, so
    generation loss here would be visible on the page.
    """
    ...
    subprocess.run(["ffmpeg", "-v", "error", "-y", "-i", dst,
                    "-frames:v", "1", "-q:v", "4",
                    os.path.join(MEDIA, name + ".jpg")], check=True)
    print("  %s film staged (%d KB)" % (name, os.path.getsize(dst) // 1024))
...
def main():
    ...
    build_grunge()
    build_grain()
    for srcfile, name in FILMS:
        stage_film(srcfile, name)
    ...
    for name, srcfile, preset, w, h, f0, f1 in CLIPS:
        ...
        if name == "hero":
            stabilise(mp4, w)
            # the webm is re-derived from the stabilised mp4 so the two cannot
            # drift apart; encoding both from scratch would double the work
            subprocess.run(["ffmpeg", "-v", "error", "-y", "-i", mp4, "-an",
                            "-c:v", "libvpx-vp9", "-crf", "26", "-b:v", "0",
                            "-row-mt", "1", "-cpu-used", "3", "-pix_fmt",
                            "yuv420p", os.path.join(MEDIA, name + ".webm")],
                           check=True)
            for f in (mp4, os.path.join(MEDIA, name + ".webm")):
                pingpong(f)
            info["frames"] = int(cv2.VideoCapture(mp4).get(cv2.CAP_PROP_FRAME_COUNT))
            info["loop"] = "ping-pong: forward then reversed, no seam"
